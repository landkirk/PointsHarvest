...
export async function run(ctx, { dailySets = [], dailySetDebug = null } = {}) {
  await ctx.setState({ dailySetDebug });
  await ctx.dbg('info', `Daily sets: ${dailySetDebug?.actionable ?? 0} actionable (section ${dailySetDebug?.sectionFound ? 'found' : 'not found'})`);
  ...
  for (let i = 0; i < dailySets.length; i++) {
    ...
    if (USER_ACTION_RE.test(tileText)) {
      ...
      await validateTile.run(ctx, dailySets[i]);
    } else {
